...
#
#  2. User-space systemd service (i2c_reset_shutdown.service)
...
install: all
	...
	install -m 644 i2c_reset_shutdown.service /etc/systemd/system/
	systemctl daemon-reload
	systemctl enable i2c_reset_shutdown.service

# uninstall: disable and remove the systemd service.
#
# Note: this does not remove the .ko from the module tree.
# Run 'modprobe -r i2c_reset' or reboot to unload the module.
uninstall:
	systemctl disable i2c_reset_shutdown.service
	rm -f /etc/systemd/system/i2c_reset_shutdown.service
	systemctl daemon-reload
